<template>
  <div class="cate-edit detail">
    <a-form ref="formRef" size="medium" :model="form" layout="horizontal" :auto-label-width="true">
      <div class="card-wrap">
        <a-card title="自定义详细信息编辑" :bordered="false">
          <a-form-item
            field="pubtime"
            label="发布时间"
            :content-flex="false"
          ...
          <a-form-item
            ...
            label="自定义标题"
            :content-flex="false"
            :rules="[{required: true, message: '请填写自定义标题', type: 'string'}]"
          >
            <a-row class="full-width">
              <a-col :span="15">
          ...
          <a-form-item label="自定义内容" :content-flex="false">
            <a-row class="full-width">
              <a-col :span="22">
...
const getDetail = async () => {
  ...
  const {code, data} = await fileCaseInit({ id: form.id, type: form.type, page_id: form.page_id })
  if (code === 0) {
...
const saveFn = () => {
  formRef.value.validate(async (err) => {
    if (!err) {
      loading.value = true
      const { id, pubtime, remark, name, type, page_id } = form
