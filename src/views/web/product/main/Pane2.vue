          ...
          <a-col :span="12">
            <a-space>
              ...
              <a-button size="small" status="danger" type="text" @click="showPopFn">
                <template #default>批量恢复</template>
              </a-button>
            </a-space>
          </a-col>
        </a-row>
...
const getTableData = async () => {
  loading.value = true
  const { search_name, category_id, p_uid, hs_code, product_id, order, order_by } = form
...
const recover = async (item) => {
  const res = await productRecover({
    product_ids: [item.id]
  })
  if (res.code == 0) {
    Message.success(res.message || '操作成功')
...
const batchDel = async () => {
  btnloading.value = true
  const res = await productRecover({ product_ids: selectObj.keys })
  btnloading.value = false
  if (res.code === 0) {
