    ...
    <div class="table-page">
        ...
        <a-row :gutter="16" wrap style="margin-bottom: 12px">
          <a-col :span="12">
            <a-space>
              ...
              <a-button size="small" status="danger" type="text" @click="showCancelPopFn">
                <template #default>批量取消</template>
              </a-button>
            </a-space>
          </a-col>
        </a-row>
        <div class="table-inner">
          ...
          >
            <template #columns>
              ...
              <a-table-column title="产品图片" data-index="name" :width="90" align="center">
              ...
              <a-table-column title="操作" :width="150" align="center" fixed="right">
                <template #cell="{ record }">
                  <a-space :size="4">
                    <!-- <a-button type="text" size="mini" @click="goEdit(record.id)">
                    ...
                    <a-popconfirm type="warning" content="您确定要取消该项吗?" @ok="cancelPro(record)">
      ...
      <a-modal v-model:visible="popupcancel" :width="286">
        ...
        <template #footer>
          ...
          <a-button type="primary" @click="batchcancel" :loading="btnloading">确定</a-button>
        </template>
      </a-modal>
      <!-- 调整负责人弹框 -->
...
const getTableData = async () => {
  loading.value = true
  const { category_id, p_uid, hs_code, product_id, order, order_by } = form
...
// 取消
const cancelPro = async (item) => {
  const res = await cancelHotspot({
    product_ids: [item.id]
  })
  if (res.code == 0) {
    Message.success(res.message || '操作成功')
...
const batchcancel = async () => {
  btnloading.value = true
  const res = await cancelHotspot({product_ids: selectObj.keys})
  if (res.code === 0) {
