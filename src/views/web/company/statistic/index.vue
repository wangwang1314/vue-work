    ...
    <div class="table-page">
      <section class="table-box">
        ...
        <div class="table-inner">
          ...
          >
            <template #columns>
              <a-table-column title="触发页面" data-index="id" :width="200" align="left">
                <template #cell="{ record }">
                  <div class="pro-id">{{ record.name }}</div>
                </template>
              </a-table-column>
              <a-table-column title="代码位置" data-index="id" :width="200" align="left">
                <template #cell="{ record }">
                  <div class="pro-id">{{ getstr(record.content) || '--' }}</div>
                </template>
              </a-table-column>
...
import { companygetstatistic } from '@/apis'
...
const getTableData = async () => {
  ...
  if (res.code == 0) {
    ...
    for (let item in keyDate) {
      keyDate[item].mainkey = item
      keyDate[item].child.forEach((list, index) => {
        if (!mainitem[list]) {
          keyDate[item].content[index] = ''
        }
      })
