      ...
      <div class="table-page">
        <div class="table-tit">
          <div class="table-tit-inner">
            <div class="icon-box">
              <img src="@/assets/images/ai/tiktok-i.png" alt="" />
            </div>
            <div class="inner-text">
              <div class="tit" v-if="reportObj.diamond">当前TikTok视频搜索前十排名量：{{ reportObj.total_records }}</div>
              <div class="tit" v-else>当前TikTok视频搜索首页排名量：{{ reportObj.total_records }}</div>
              <div class="sub-tit">
                <span v-if="reportObj.diamond">前三排名量: {{ reportObj.total_three }}</span>
                <!-- <span>已发布TikTok视频量: {{ reportObj.video_total }}</span> -->
              </div>
            </div>
          </div>
        </div>
        <div class="table-inner">
          <a-row>
            <a-col :span="12">
              <div class="col-tit">TikTok排名</div>
            </a-col>
            <a-col :span="12">
              <div class="col-right">
        ...
        <div class="alert-box">
          <a-alert type="warning">
            ...
            <ul class="alert-content">
              <li>1、以tiktok.com英文版北美地域排名为检测标准，定期提供关键词报告。</li>
              <li>
...
const downloadFn = () => {
  window.location.href =
    ...
    VITE_API_PREFIX +
    '/index.php?r=' + textstr + '/download-video-report&report_time=' +
    reportObj.value.report_time + '&type=' + ptype.value + (cid?'&cid=' + cid:'')
}
onMounted(() => {
  getTableData()
})
...
.table-tit {
  ...
  background: url('@/assets/images/ai/tiktok.png') center right no-repeat;
  .table-tit-inner {
    display: flex;
    align-items: center;
    .icon-box {
