<template>
  <div class="login">
    <section class="login-box animated animate__headShake">
      <!-- 左侧 -->
      ...
      <div class="login-right">
        ...
        >
          <h3 class="login-form-title"><img class="logo" src="@/assets/images/logo.gif" /><span>ECWEB</span></h3>
...
const locaForm = JSON.parse(localStorage.getItem('userlogin') || '{}')
const locaChecked = Boolean(localStorage.getItem('remember'))
const form: LoginParams = reactive({
  username: locaForm.username || '',
  password: locaForm.password || ''
})

// 记住密码
const checked = ref(locaChecked)
// 登录加载
...
const login = async () => {
  try {
    ...
    if (checked.value) {
      localStorage.setItem('userlogin', JSON.stringify(form))
    } else {
      localStorage.removeItem('userlogin')
    }
    localStorage.setItem('remember', checked.value ? 'true' : '')
    router.push('/')
    setLoading(false)
    navTabStore.init()
