<template>
  <a-layout-header>
    ...
    <a-space class="system-head" v-if="route.name === 'inquiryDetail' && fileStore.status == true">
      商机置顶 <a-switch @click="save" v-model="fileStore.loading" />
    </a-space>
    <a-space class="system-head" v-if="route.name === 'inquiryReply'">
      <a-button type="primary" @click="save" :loading="fileStore.loading">
        <template #icon><icon-email :size="13" :stroke-width="3" /></template>
        发送</a-button
      >
      <a-button @click="cancel" :disabled="fileStore.loading">
        <template #icon><icon-close :size="13" :stroke-width="3" /></template>
        取消</a-button
      >
    </a-space>
  </a-layout-header>
...
<script setup lang="ts" name="inquiryHeader">
...
const nogobackarr = ref(['inquiryDetail'])
</script>
